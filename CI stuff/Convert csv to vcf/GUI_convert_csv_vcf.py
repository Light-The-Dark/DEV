...
# Dictionary to map display names to functions (kept for future use)
CONVERSION_METHODS = {
    "Default": "default",
...
def convert():
    ...
    if not csv_file or not contacts_save_path:
        messagebox.showerror("Error", "Please fill in all fields")
        return

    try:
        vcard_file = os.path.join(contacts_save_path, "~vcard.vcf")
        result_file = csv_to_vcard(csv_file, vcard_file)
        print(f"Created: {result_file}")
        messagebox.showinfo("Success", f"VCard file created successfully: {result_file}")
...
tk.Label(main_frame, text="CSV File:").grid(row=0, column=0, sticky="e", padx=(10, 5), pady=3)
csv_entry = tk.Entry(main_frame, width=50)
csv_entry.grid(row=0, column=1, sticky="w", padx=(0, 5), pady=3)
csv_entry.insert(0, get_default_csv_path())
tk.Button(main_frame, text="Browse", command=browse_csv).grid(row=0, column=2, sticky="w", padx=(0, 10), pady=3)

tk.Label(main_frame, text="Save Path:").grid(row=1, column=0, sticky="e", padx=(10, 5), pady=3)
save_path_entry = tk.Entry(main_frame, width=50)
save_path_entry.grid(row=1, column=1, sticky="w", padx=(0, 5), pady=3)
save_path_entry.insert(0, os.path.join(os.path.expanduser("~"), "Desktop"))
tk.Button(main_frame, text="Browse", command=browse_save_path).grid(row=1, column=2, sticky="w", padx=(0, 10), pady=3)
